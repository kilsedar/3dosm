...
define(['libraries/WebWorldWind/src/formats/geojson/GeoJSONParser',
        ...
        'osmtogeojson'],
       function (GeoJSONParser, ArgumentError, Logger, osmtogeojson) {
  "use strict";
  ...
  OSMLayer.prototype.loadByBoundingBox = function () {
    ...
    for (var typeIndex = 0; typeIndex < this.type.length; typeIndex++) {
      // console.log(this.type[typeIndex]);
      data += this.type[typeIndex] + '[' + this.tag + '](' + this.boundingBox[1] + ',' + this.boundingBox[0] + ',' + this.boundingBox[3] + ',' + this.boundingBox[2] + '); ';
    }
    ...
    return $.ajax({
      ...
      success: function(dataOverpass) {
        ...
        _self.data = dataOverpassGeoJSON;
        _self.dataSize = JSON.stringify(dataOverpassGeoJSON).length;
      },
      error: function(e) {
        throw new ArgumentError(
  ...
  OSMLayer.prototype.loadByGeoJSONFile = function () {
    ...
    return $.ajax({
      ...
      success: function(data) {
        ...
        _self.data = data;
        _self.dataSize = JSON.stringify(data).length;
      },
      error: function(e) {
        throw new ArgumentError(
  ...
  OSMLayer.prototype.loadByGeoJSONData = function () {
    ...
    this.dataSize = JSON.stringify(this.source.data).length;
  };
